...
parser.add_argument('--delete_all_files')
...
for row in grist_data['records']:
    ...
    filenames = str(row['fields'][args.grist_column_filenames]).split(',') if args.grist_column_filenames in row['fields'] else None
    sherlock_uuid = row['fields'][args.grist_column_sherlock_uuid]

    print("-" * 111)
    print("💬", f"{round(i / len(grist_data['records']) * 100, 2)}% ", '🆔', business_id)
