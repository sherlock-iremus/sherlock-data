...
transport = AIOHTTPTransport(
    url=secret["url"] + '/graphql' + '?access_token=' + access_token)
...
while True:
    ...
    for ensemble in response["ensembles"]:
        ...
        for membre in ensemble["membres"]:
            t(E74_uri, crm("P107_has_current_or_former_member"),
              she(membre["id"]))

        E41_uri = she(cache.get_uuid(
            ["ensembles", E74_uri, "E41", "uuid"], True))
...
while True:
    ...
    for institution in response["institutions"]:
        ...
        E41_uri = she(cache.get_uuid(
            ["institutions", E39_uri, "E41", "uuid"], True))
...
while True:
    ...
    for lieu in response["lieux_de_representation"]:
        ...
        E41_uri = she(cache.get_uuid(
            ["lieux de représentation", lieu_uri, "E41", "uuid"], True))
...
while True:
    ...
    for maison in response["maisons_d_edition"]:
        ...
        E41_uri = she(cache.get_uuid(
            ["maisons d'édition", E39_uri, "E41", "uuid"], True))
        t(E39_uri, crm("P1_is_identified_by"), E41_uri)
        t(E41_uri, a, crm("E41_Appellation"))
...
while True:
    ...
    for oeuvre in response["oeuvres_litteraires"]:
        ...
        E35_uri = she(cache.get_uuid(
            ["oeuvres littéraires", F2_uri, "E35", "uuid"], True))
        ...
        F28_uri = she(cache.get_uuid(
            ["oeuvres littéraires", F2_uri, "F28", "uuid"], True))
        ...
        if oeuvre["auteurs"] != None:
            for auteur in oeuvre["auteurs"]:
                t(F28_uri, crm("P14_carried_out_by"),
                  she(auteur["personne_id"]["id"]))

        # F3 Manifestation
        F3_uri = she(cache.get_uuid(
            ["oeuvres littéraires", F2_uri, "F3", "uuid"], True))
        t(F3_uri, a, lrm("F3_Manifestation"))
        t(F3_uri, lrm("R4_embodies"), F2_uri)

        # F30 Manifestation Creation
        F30_uri = she(cache.get_uuid(
            ["oeuvres littéraires", F2_uri, "F3", "F30", "uuid"], True))
        ...
        if oeuvre["date_de_publication"]:
            E52_uri = she(cache.get_uuid(
                ["oeuvres littéraires", F2_uri, "F3", "F30", "E52", "uuid"], True))
            t(E52_uri, a, crm("E52_Time-Span"))
            date = oeuvre["date_de_publication"]
            t(E52_uri, crm("P82_at_some_time_within"), l(
                f"{date}-01-01T00:00:00Z", datatype=XSD.dateTime))
...
while True:
    ...
    for oeuvre in response["oeuvres_musicales"]:
        ...
        t(F2_uri, a, lrm("F2_Expression"))
        E35_uri = she(cache.get_uuid(
            ["oeuvres musicales", F2_uri, "E41", "uuid"], True))
        t(F2_uri, crm("P102_has_title"), E35_uri)
        t(E35_uri, a, crm("E35_Title"))
        t(E35_uri, crm("P190_has_symbolic_content"), l(oeuvre["titre"]))
        ...
        # F28 Expression Creation
        F28_uri = she(cache.get_uuid(
            ["oeuvres musicales", F2_uri, "F28", "uuid"], True))
        t(F28_uri, a, lrm("F28_Expression_Creation"))
        t(F28_uri, lrm("R17_created"), F2_uri)

        # Sous-F28 Composition
        if oeuvre["compositeurs"] != None:
            sous_F28_uri = she(cache.get_uuid(
                ["oeuvres musicales", F2_uri, "F28", "compositeurs", "uuid"], True))
            t(sous_F28_uri, a, lrm("F28_Expression_Creation"))
            t(sous_F28_uri, crm("P2_has_type"), she(
                "1d2f3f12-5bde-492e-a8ba-a3ac50652758"))
            ...
            t(
                ...
                l(f"{oeuvre['date_de_composition']}-01-01T00:00:00Z",
                  datatype=XSD.dateTime)
            )

            # Sous-F28 Ecriture électronique (intégré au sous-F28 Composition)
            if oeuvre["usage_de_l_electronique"] == True:
                electro_uri = she(cache.get_uuid(
                    ["oeuvres musicales", F2_uri, "F28", "électronique", "uuid"], True))
                t(electro_uri, a, lrm("F28_Expression_Creation"))
                t(electro_uri, crm("P2_has_type"), she(
                    "caf3ea36-ae41-42dd-a7ed-d7c2405f18a5"))
                t(electro_uri, crm("P14_carried_out_by"),
                  she(compositeur["personne_id"]["id"]))
                ...
                if oeuvre["responsables_de_l_electronique"] != None:
                    for institution in oeuvre["responsables_de_l_electronique"]:
                        t(electro_uri, crm("P14_carried_out_by"),
                          she(institution["institution_id"]["id"]))

        # Sous-F28 Libretto
        if oeuvre["librettistes"] != None:
            sous_F28_uri = she(cache.get_uuid(
                ["oeuvres musicales", F2_uri, "F28", "librettistes", "uuid"], True))
            t(sous_F28_uri, a, lrm("F28_Expression_Creation"))
            t(sous_F28_uri, crm("P2_has_type"), she(
                "5cdb2ecf-c05e-4b97-a51f-49e597e89f54"))
            t(F28_uri, crm("P9_consists_of"), sous_F28_uri)
            for librettiste in oeuvre["librettistes"]:
                t(sous_F28_uri, crm("P14_carried_out_by"),
                  she(librettiste["personne_id"]["id"]))

        # Numéro d'ordre dans l'oeuvre composite
        if oeuvre["numero_d_ordre_dans_oeuvre_composite"] != None:
            t(F2_uri, dor("U10_has_order_number"), l(
                oeuvre["numero_d_ordre_dans_oeuvre_composite"]))

        # Durée de l'oeuvre
        if oeuvre["duree_en_min"] != None:
            E54_uri = she(cache.get_uuid(
                ["oeuvres musicales", F2_uri, "E54", "uuid"], True))
        ...
        if oeuvre["effectifs"] != None:
            M6_uri = she(cache.get_uuid(
                ["oeuvres musicales", F2_uri, "M6", "uuid"], True))
            t(M6_uri, a, dor("M6_Casting"))
            t(F2_uri, dor("U13_has_casting"), M6_uri)

            for instrument in oeuvre["effectifs"]:
                M23_uri = she(cache.get_uuid(
                    ["oeuvres musicales", F2_uri, "M6", "M23", instrument["voix_et_instrument_id"]["nom"], "uuid"], True))
                t(M23_uri, a, dor("M23_Casting_Detail"))
                t(M6_uri, dor("U23_has_casting_detail"), M23_uri)
                t(M23_uri, dor("U2_foresees_use_of_medium_of_performance"),
                  she(instrument["voix_et_instrument_id"]["id"]))
                if instrument["nombre"] != None:
                    t(M23_uri, dor("U30_foresees_quantity_of_medium_of_performance"), l(
                        instrument["nombre"]))

        # Source littéraire
        if oeuvre["sources_litteraires"] != None:
            for source in oeuvre["sources_litteraires"]:
                t(F2_uri, crm("P130_shows_features_of"), she(
                    source["oeuvre_litteraire_id"]["id"]))
        ...
        if len(oeuvre["producteurs"]) >= 1:
            commande_uri = she(cache.get_uuid(
                ["oeuvres musicales", F2_uri, "producteur", "uuid"], True))
            t(commande_uri, a, she_ns("Commission"))
            t(commande_uri, she_ns("commission_of"), F2_uri)
            for compositeur in oeuvre["compositeurs"]:
                t(commande_uri, she_ns("commission_received_by"),
                  she(compositeur["personne_id"]["id"]))
            for producteur in oeuvre["producteurs"]:
                t(commande_uri, crm("P14_carried_out_by"),
                  she(producteur["item"]["id"]))
...
while True:
    ...
    for oeuvre in response["oeuvres_musicales_oeuvres_composites"]:
        ...
        if oeuvre["autonome"] == True or oeuvre["autonome"] == None:
            t(she(oeuvre["oeuvre_composite_id"]["id"]),
              crm("P165_incorporates"), F2_uri)

        if oeuvre["autonome"] == False:
            t(she(oeuvre["oeuvre_composite_id"]["id"]),
              crm("P148_has_component"), F2_uri)

        if oeuvre["commentaire"] != None:
            E13_uri = she(cache.get_uuid(
                ["oeuvres musicales", F2_uri, "commentaire sous-oeuvre", "E13", "uuid"], True))
            t(E13_uri, a, crm("E13_Attribute_Assignement"))
            t(E13_uri, crm("P14_carried_out_by"), she(
                "60324ad9-35a3-489f-a203-5aecc435f7aa"))
...
while True:
    ...
    for partition in response["partitions"]:
        ...
        if partition["edition"] != None:
            F30_uri = she(cache.get_uuid(
                ["partitions", F3_uri, "F30", "uuid"], True))
            t(F30_uri, a, lrm("F30_Manifestation_Creation"))
            t(F30_uri, lrm("R24_created"), F3_uri)
            for editeur in partition["edition"]:
                t(F30_uri, crm("P14_carried_out_by"),
                  she(editeur["item"]["id"]))
    ...
    if representation[champ] != None and len(representation[champ]) >= 1:
        ...
        if nom == "chef d'orchestre":
            M28_uri = she(cache.get_uuid(
                ["representations", F31, nom, "M28", representation[champ]["Nom"], "uuid"], True))
            t(M28_uri, a, dor("M28_Individual_Performance"))
            t(M42_uri, crm("P9_consists_of"), M28_uri)
            t(M28_uri, crm("P14_carried_out_by"),
              she(representation[champ]["id"]))
        else:
            for i in representation[champ]:
                M28_uri = she(cache.get_uuid(
                    ["representations", F31, nom, "M28", i["personne_id"]["Nom"], "uuid"], True))
                t(M28_uri, a, dor("M28_Individual_Performance"))
                t(M42_uri, crm("P9_consists_of"), M28_uri)
                t(M28_uri, crm("P14_carried_out_by"),
                  she(i["personne_id"]["id"]))
...
def create_E29(nom, champ, P2_type):
    if len(representation[champ]) >= 1:
        E29_uri = she(cache.get_uuid(
            ["performance plan", F31_serie_uri, nom, "uuid"], True))
        ...
        E29_F28_uri = she(cache.get_uuid(
            ["performance plan", F31_serie_uri, nom, "F28", "uuid"], True))
        t(E29_F28_uri, a, lrm("F28_Expression_Creation"))
        t(E29_F28_uri, lrm("R17_created"), E29_uri)
        for i in representation[champ]:
            t(E29_F28_uri, crm("P14_carried_out_by"),
              she(i["personne_id"]["id"]))
...
while True:
    ...
    for representation in response["representations"]:
        ...
        t(F31_serie_uri, crm("P2_has_type"), she(
            "ffc01a36-69b8-4ca2-9048-714961d1397b"))
        t(F31_serie_uri, lrm("R66_included_performed_version_of"),
          she(representation["oeuvre_musicale"]["id"]))
        ...
        pp_uri = she(cache.get_uuid(
            ["performance plan", F31_serie_uri, "uuid"], True))
        ...
        t(pp_uri, crm("P165_incorporates"), she(
            representation["oeuvre_musicale"]["id"]))

        t(F31_serie_uri, lrm("R66_included_performed_version_of"), pp_uri)

        # Chorégraphie
        create_E29("chorégraphie", "choregraphie",
                   "561e8ef8-6786-4df4-8cd5-c74479860753")

        # Mise en scène
        create_E29("mise en scène", "mise_en_scene",
                   "ff33c9d3-8e91-4b1e-ba5c-1d7f4c8c34bd")

        # Scénographie
        create_E29("scénographie", "scenographie",
                   "e5e5890d-d774-43cc-b573-e1dc871bd05b")

        # Dramaturgie
        create_E29("dramaturgie", "dramaturgie",
                   "12597890-dcac-42f4-b780-d9795364ed74")

        # Dialogues
        create_E29("dialogues", "dialogues",
                   "63da5276-9d37-4e5f-9d40-b16723c42ed2")

        # Conseil gastronomique à ignorer pour l'instant - pas de données

        # Costumes
        create_E29("costumes", "costumes",
                   "181dc490-d221-41e8-81e6-1de21326c15f")
        ...
        if len(representation["dates"]) >= 1:
            for date in representation["dates"]:
                ...
                # Date
                E52_uri = she(cache.get_uuid(
                    ["representations", F31_uri, "E52", "uuid"], True))
                t(F31_uri, crm("P4_has_time-span"), E52_uri)
                date_iso = date["date"]
                t(E52_uri, crm("P82_at_some_time_within"), l(
                    f"{date_iso}T00:00:00Z", datatype=XSD.dateTime))
                ...
                # Effectif
                M42_uri = she(cache.get_uuid(
                    ["representations", F31_uri, "effectif", "uuid"], True))
                t(M42_uri, a, dor("M42_Performed_Expression_Creation"))
                t(M42_uri, crm("P2_has_type"), she(
                    "c0e9693e-3e60-4d89-b005-56dcd864180d"))
                t(F31_uri, crm("P9_consists_of"), M42_uri)
                M43_uri = she(cache.get_uuid(
                    ["representations", F31_uri, "effectif", "M43", "uuid"], True))
                t(M43_uri, a, dor("M43_Performed_Expression"))
                t(M42_uri, lrm("R17_created"), M43_uri)
                t(M43_uri, dor("U54_is_performed_expression_of"),
                she(representation["oeuvre_musicale"]["id"]))
                    ...
                    for effectif in representation["effectif"]:
                        ...
                        if effectif["nombre"] != None and effectif["nombre"] >= 2:
                            ...
                            for i in range(nombre):
                                M28_uri = she(cache.get_uuid(
                                    ["representations", F31_uri, "effectif", "M28", effectif["voix_et_instrument_id"]["nom"], i, "uuid"], True))
                                t(M28_uri, a, dor("M28_Individual_Performance"))
                                t(M42_uri, crm("P9_consists_of"), M28_uri)
                ...
                # Interprètes
                create_M42_M43_M28(
                    F31_uri, "interprétation", "interpretes", "8d5b9d50-27a1-488a-82a6-19ee59acd847")
                ...
                # Eclairages
                create_M42_M43_M28(
                    F31_uri, "éclairages", "eclairages", "8e27d9eb-1b30-4b14-98a8-71d48a5b3a4d")
                ...
                # Maquillage
                create_M42_M43_M28(
                    F31_uri, "maquillage", "maquillage", "6a9dc63f-f6f0-467f-9703-864e7781b47d")

                # Bande son
                create_M42_M43_M28(
                    F31_uri, "bande son", "bande_son", "4b774fbe-6557-4589-9ae2-f9965e8e9bba")
                ...
                # Chef d'orchestre
                create_M42_M43_M28(
                    F31_uri, "chef d'orchestre", "chef_d_orchestre", "bf07dbd8-09ef-4152-bd6e-570984ec294f")

                # Sonorisation
                create_M42_M43_M28(
                    F31_uri, "sonorisation", "sonorisation", "d0bf0c77-1a79-4a66-9679-4cd8326cd226")

                # Marionnettes
                create_M42_M43_M28(
                    F31_uri, "marionnettes", "marionnettes", "92b8d777-0495-4482-ad2d-f17c9ea682d1")

                # Images et vidéo
                create_M42_M43_M28(
                    F31_uri, "images et vidéo", "images_et_video", "1f9b29ea-3da4-4974-99ca-e01bbf2bb9e4")
        ...
        # 2. Association des champs de la table "representations" à la série de représentations
            ...
            if representation["date_de_debut"] != None and representation["date_de_fin"] != None:
                E52_uri = she(cache.get_uuid(
                    ["representations", F31_serie_uri, "E52", "uuid"], True))
                t(E52_uri, a, crm("E52_Time-Span"))
                date_debut_iso = representation["date_de_debut"]["date"]
                date_fin_iso = representation["date_de_fin"]["date"]
                t(E52_uri, crm("P79_beginning_is_qualified_by"), l(
                    f"{date_debut_iso}-01-01T00:00:00Z", datatype=XSD.dateTime))
                t(E52_uri, crm("P80_end_is_qualified_by"), l(
                    f"{date_fin_iso}-01-01T00:00:00Z", datatype=XSD.dateTime))
                t(F31_serie_uri, crm("P4_has_time-span"), E52_uri)

                # Lieu
                t(F31_serie_uri, crm("P7_took_place_at"),
                  she(representation["lieu"]["id"]))

                # Effectif
                M42_uri = she(cache.get_uuid(
                    ["representations", F31_serie_uri, "effectif", "uuid"], True))
                t(M42_uri, a, dor("M42_Performed_Expression_Creation"))
                t(M42_uri, crm("P2_has_type"), she(
                    "c0e9693e-3e60-4d89-b005-56dcd864180d"))
                t(F31_serie_uri, crm("P9_consists_of"), M42_uri)
                M43_uri = she(cache.get_uuid(
                    ["representations", F31_serie_uri, "effectif", "M43", "uuid"], True))
                t(M43_uri, a, dor("M43_Performed_Expression"))
                t(M42_uri, lrm("R17_created"), M43_uri)
                t(M43_uri, dor("U54_is_performed_expression_of"),
                  she(representation["oeuvre_musicale"]["id"]))
                if len(representation["effectif"]) >= 1:
                    for effectif in representation["effectif"]:
                        ...
                        if effectif["nombre"] != None and effectif["nombre"] >= 2:
                            ...
                            for i in range(nombre):
                                M28_uri = she(cache.get_uuid(
                                    ["representations", F31_serie_uri, "effectif", "M28", effectif["voix_et_instrument_id"]["nom"], i, "uuid"], True))
                                t(M28_uri, a, dor("M28_Individual_Performance"))
                                t(M42_uri, crm("P9_consists_of"), M28_uri)
                                t(M28_uri, dor("U1_used_medium_of_performance"), she(
                                    effectif["voix_et_instrument_id"]["id"]))
                            ...
                            M28_uri = she(cache.get_uuid(
                                ["representations", F31_serie_uri, "effectif", "M28", effectif["voix_et_instrument_id"]["nom"], "uuid"], True))
                            t(M28_uri, a, dor("M28_Individual_Performance"))
                            t(M42_uri, crm("P9_consists_of"), M28_uri)
                            t(M28_uri, dor("U1_used_medium_of_performance"),
                              she(effectif["voix_et_instrument_id"]["id"]))
                ...
                # Interprètes
                create_M42_M43_M28(F31_serie_uri, "interprétation",
                                   "interpretes", "8d5b9d50-27a1-488a-82a6-19ee59acd847")

                # Danseurs
                create_M42_M43_M28(
                    F31_serie_uri, "danse", "danseurs", "68b77a08-d6c8-42ed-bd3d-a922ec52b064")

                # Eclairages
                create_M42_M43_M28(
                    F31_serie_uri, "éclairages", "eclairages", "8e27d9eb-1b30-4b14-98a8-71d48a5b3a4d")

                # Régie
                create_M42_M43_M28(F31_serie_uri, "régie", "regie",
                                   "ed23180f-5cd4-448a-9852-4078aeebc53b")

                # Maquillage
                create_M42_M43_M28(
                    F31_serie_uri, "maquillage", "maquillage", "6a9dc63f-f6f0-467f-9703-864e7781b47d")

                # Bande son
                create_M42_M43_M28(
                    F31_serie_uri, "bande son", "bande_son", "4b774fbe-6557-4589-9ae2-f9965e8e9bba")

                # Direction musicale
                create_M42_M43_M28(F31_serie_uri, "direction musicale",
                                   "direction_musicale", "1d0cccc9-35a6-4847-80a5-ad48a6fefe6b")

                # Chef d'orchestre
                create_M42_M43_M28(F31_serie_uri, "chef d'orchestre",
                                   "chef_d_orchestre", "bf07dbd8-09ef-4152-bd6e-570984ec294f")

                # Sonorisation
                create_M42_M43_M28(F31_serie_uri, "sonorisation",
                                   "sonorisation", "d0bf0c77-1a79-4a66-9679-4cd8326cd226")

                # Marionnettes
                create_M42_M43_M28(F31_serie_uri, "marionnettes",
                                   "marionnettes", "92b8d777-0495-4482-ad2d-f17c9ea682d1")

                # Images et vidéo
                create_M42_M43_M28(F31_serie_uri, "images et vidéo",
                                   "images_et_video", "1f9b29ea-3da4-4974-99ca-e01bbf2bb9e4")

                # Direction technique
                create_M42_M43_M28(F31_serie_uri, "direction technique",
                                   "direction_technique", "4583a259-9bfa-4275-bb17-06d0d1c28779")
