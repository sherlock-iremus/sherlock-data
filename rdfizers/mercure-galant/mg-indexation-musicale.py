...
for row in rows:
    ...
    cell = row["mots-clefs vérif : Anne"]

    if cell != None:
        if ";" in cell:
            liste_mots_clefs = cell.split(";")
            for mot_clef in liste_mots_clefs:
                if mot_clef != " " and mot_clef != "":
                    mot_clef = mot_clef.strip()
                    if mot_clef not in mots_clefs:
                        mots_clefs.append(mot_clef)
        elif cell not in mots_clefs:
            mots_clefs.append(cell.strip())
        else:
            pass
